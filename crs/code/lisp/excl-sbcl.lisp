;; This is a very thin layer to make SBCL's functions look like Allegro's EXCL pkg.

;;; ccl's run-program wants args to be a list of strings.  This is
;;; diff from ACL which handles a vector, list of strings, or single
;;; string with space-separated args all correctly, afaict.

;;; here we expect that the caller is leveraging the last option
;;; above, a single space-sep string, and we'll break that into a list
;;; of no-space strings.  Heaven forfend you have quoted strings in
;;; there...  oh wait yeah we have to handle that.... nevermind, let's
;;; just fix our callers.

#-sbcl (error "This should only load in SBCL")
