import os
from modules.rewrite_func_given_sanitizer_blob_harness import rewrite_func_given_sanitizer_blob_harness
...
def generate_patch(head_sanitizer_output, head_ftp_code, code_base_path, ftp_path, vuln, harness, bic_sanitizer_output, bic_ftp_code, bic_delta):
    print("full_scan_rewrite_func_given_sanitizer_blob_harness", len(head_ftp_code), len(head_sanitizer_output), len(vuln), len(harness))
    
    # Write a new file
    new_code = rewrite_func_given_sanitizer_blob_harness(head_ftp_code, head_sanitizer_output, vuln, harness)
