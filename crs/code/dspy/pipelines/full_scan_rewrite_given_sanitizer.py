import os
from modules.rewrite_code_given_sanitizer import rewrite_code_given_sanitizer
...
def generate_patch(head_sanitizer_output, head_ftp_code, code_base_path, ftp_path, vuln, harness, bic_sanitizer_output, bic_ftp_code, bic_delta):
    print("full_scan_rewrite_given_sanitizer", len(head_ftp_code), len(head_sanitizer_output))
    # Write a new file
    new_code = rewrite_code_given_sanitizer(head_ftp_code, head_sanitizer_output)

    record_before_and_after(head_ftp_code, new_code)
