...
def generate_patch(head_sanitizer_output, head_ftp_code, code_base_path, ftp_path, vuln, harness, bic_sanitizer_output, bic_ftp_code, bic_delta):
    print("delta_scan_rewrite_func_given_sanitizer", len(head_ftp_code), len(head_sanitizer_output), len(bic_delta))
    ...
    # Write a new file
    new_code = rewrite_func_given_sanitizer_delta(head_ftp_code, head_sanitizer_output, bic_delta, hints=hints)
